...
4. Create a new class (with namespace of Keyfactor.Extensions.Orchestrator.RemoteFile.{NewType}) in the new folder that will implement ICertificateStoreSerializer.  By convention, {StoreTypeName}CertificateSerializer would be a good choice for the class name.  This interface requires you to implement four methods:
  * HasPrivateKeyOverride - Alternate logic to determine if private key exists if the file-based private key cannot be read.  In most cases, this is not needed, so just return False.
  * GetPrivateKeyPath - Only valid if the private key is stored seperately.  In most cases, this is not needed, so just return NULL.
  * DesrializeRemoteCertificateStore - Based on a byte array containing the contents of file based store you are managing, convert this to an Org.BouncyCastle.Pkcs.Pkcs12Store class and return it.
  * SerializeRemoteCertificateStore - Takes in an Org.BouncyCastle.Pkcs.Pkcs12Store and converts it to a collection of custom file representations, List<SerializedStoreInfo>.
