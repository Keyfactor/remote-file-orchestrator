...
The RFKDB Certificate Store Type is designed to manage IBM Key Database Files (KDB), providing a robust and efficient way to handle certificates within IBM's ecosystem. By integrating RFKDB with the Remote File Universal Orchestrator extension, users can perform inventory, add, remove, create, and discovery operations on KDB files, ensuring streamlined and comprehensive certificate management.

### Representation and Use Cases

The RFKDB Certificate Store represents IBM Key Database Files, which are used to store digital certificates and their associated private keys within IBM environments. This store type supports several key use cases:
...
There are several important caveats and limitations to be aware of when using the RFKDB Certificate Store Type:

1. **Dependency on IBM Utility**: The RFKDB store type requires the IBM utility, GSKCAPICMD, to be installed on the server. This utility is essential for reading and writing certificates to and from the KDB file. The location of GSKCAPICMD must be included in the system's PATH.

2. **Security Context and Permissions**: When orchestrating management tasks, the security context is determined by the user ID entered in the Keyfactor Command certificate store or discovery job screens. When running locally as an agent, the security context is the user ID running the Keyfactor Command Universal Orchestrator service account. Ensure that appropriate permissions and configurations are in place to allow the required operations.

3. **Elevated Permissions on Linux**: If managing KDB files on Linux servers, it may be necessary to configure the server for sudo access without a password, depending on the operational needs and elevated permission requirements. This involves setting the `UseSudo` value in the config.json file to 'Y'. Commands like `chown`, `cp`, and `rm` will require the appropriate permissions to execute successfully.

By understanding these key points, users can effectively leverage the RFKDB Certificate Store Type to manage IBM Key Database Files within their Keyfactor Command environment, ensuring secure and efficient certificate handling.
...
The recommended method for creating the `RFKDB` Certificate Store Type is to use [kfutil](https://github.com/Keyfactor/kfutil). After installing, use the following command to create the `` Certificate Store Type:
...
| Client Machine | For the Client Machine field when instantiating new Certificate Stores of type 'RFKDB', enter the DNS name or IP address of the remote orchestrated server. For Windows servers, use the format protocol://dns-or-ip:port (e.g., https://myserver.mydomain.com:5986). | |
| Store Path | For the Store Path field when instantiating new Certificate Stores of type 'RFKDB', enter the full path and file name, including the file extension, of the IBM Key Database file. For example, on Linux: /folder/path/keystore.kdb, and on Windows: C:\folder\path\keystore.kdb. | |
