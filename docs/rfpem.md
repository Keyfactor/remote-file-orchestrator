...
| Client Machine | The Client Machine field for Certificate Stores of type 'RFPEM' should be the DNS name or IP address of the remote orchestrated server. Example for Windows: 'https://myserver.mydomain.com:5986'. | |
| Store Path | The Store Path field for Certificate Stores of type 'RFPEM' should include the full path and file name, along with the file extension if applicable. Example for Linux: '/folder/path/certificate.pem'. | |
