...
The RFPEM Certificate Store Type is designed to manage PEM encoded files. This store type supports a variety of use cases, making it a versatile option for securely storing and managing cryptographic certificates. By integrating RFPEM with the Remote File Universal Orchestrator extension, users can perform inventory, add, remove, create, and discovery operations on PEM encoded certificate stores, ensuring efficient and effective certificate lifecycle management.

### Representation and Use Cases

The RFPEM Certificate Store represents PEM (Privacy-Enhanced Mail) encoded files, which are a widely used format for storing certificates and keys. This store type supports several use cases:

1. **Trust Stores**: Files containing one-to-many certificates without private keys or certificate chains.
2. **Single Certificate Stores**: Files containing a single certificate with a private key, with or without a certificate chain. The private key can either be stored in the same file or in an external file.

### Important Considerations

There are several important caveats and limitations to be aware of when using the RFPEM Certificate Store Type:

1. **Single Private Key Limitation**: PEM stores managed by RFPEM can only have one private key associated with the store, whether it is internal or external. This means only one certificate/chain/private key combination can be stored in a PEM file.

2. **Private Key Formats**: RFPEM supports both PKCS#1 (BEGIN RSA PRIVATE KEY) and PKCS#8 (BEGIN [ENCRYPTED] PRIVATE KEY) formatted private keys. However, for PKCS#1 formatted keys, the store password must be set to "no password" as PKCS#1 does not support encrypted keys.

3. **Security Context**: When orchestrating management tasks, the security context is determined by the user ID entered in the Keyfactor Command certificate store or discovery job screens. When running locally as an agent, the security context is the user ID running the Keyfactor Command Universal Orchestrator service account.

4. **Inventory Implications**: If the store is configured with the `IgnorePrivateKeyOnInventory` option set to true, inventory operations will be performed without accessing the certificate's private key or store password. This makes the store inventory-only, with all certificates returned as "Private Key Entry" set to false.

By understanding these key points, users can effectively leverage the RFPEM Certificate Store Type to manage PEM encoded certificates within their Keyfactor Command environment, ensuring secure and efficient certificate handling.
...
The recommended method for creating the `RFPEM` Certificate Store Type is to use [kfutil](https://github.com/Keyfactor/kfutil). After installing, use the following command to create the `` Certificate Store Type:
...
| Client Machine | For the Client Machine field when instantiating new Certificate Stores of type 'RFPEM', enter the DNS name or IP address of the remote orchestrated server. For Windows servers, use the format protocol://dns-or-ip:port (e.g., https://myserver.mydomain.com:5986). | |
| Store Path | For the Store Path field when instantiating new Certificate Stores of type 'RFPEM', enter the full path and file name, including the file extension, of the PEM encoded file. For example, on Linux: /folder/path/certificate.pem, and on Windows: C:\folder\path\certificate.pem. | |
